...
### 2023.12.21
休眠大成功！让10s计时、键鼠缓冲在没有任务时休眠效果拔群！在有两个任务一直执行的情况下性能计数达到了大约0.46B/3s（在关中断解决临界区问题的情况下），10s性能达到了1.53B，是原来的两倍多（0.7B/10s）！而且键鼠输入还是保持比较流畅，很棒！！

### 2023.12.24
鼠标点击拖动窗口状态实现。但是遇到了一些问题：
1. 拖动窗口窗口渲染事闪烁
2. 拖动窗口的距离和鼠标移动距离不同，有错位
闪烁问题通过修改了一下slide渲染算法大致解决了，但是错位还是存在。不太想牺牲鼠标的响应性（同步，让鼠标等渲染）去解决，还需要想一些更好分办法。
对于鼠标监视直接控制窗口的方法，以后可以改进成窗口消息队列的机制